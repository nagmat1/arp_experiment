...
control MyIngress(inout headers hdr,
    ...
    action arp_forward_action(egressSpec_t port) {
        standard_metadata.egress_spec = port;
    }

    action arp_mod_action(macAddr_t macAddr) {
   ...
   table arp_forward {
	...
	if (((hdr.arp.isValid()) && (hdr.arp.op == ARP_OPER_REQUEST || hdr.arp.op == ARP_OPER_REPLY))) 
		{ 
			arp_forward.apply(); 
			if (hdr.arp.op == ARP_OPER_REQUEST) {arp_mod.apply();} 
		}
